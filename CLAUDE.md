...
- `cards`: belong to an account; `type` is `debito`, `credito` (has optional `cutDay`/`payDay` statement days 1–31, and optional `limit` — the credit line; `creditUsage` derives debt/limit and Resumen shows a green→amber→red usage bar with a ⚠ warning+advice when high), `ahorro` (savings, has `rate` % and `lastAccrual`), `efectivo` (auto-created for cash accounts; cannot go negative — `cashOverdraft` blocks any movement/transfer/payment that would overdraw it and suggests creating a `deuda`), or `deuda` (auto-created for debt accounts; optional `rate` % + `ratePeriod` `"mensual"`/`"anual"` — interest accrues daily onto the debt via `applyInterest`, and the initial amount is an `adjust` gasto so it never counts as monthly spending). Debit/credit cards may also carry `digitalLast4` — the last 4 digits of an associated digital card, used (alongside `last4`) to match captured notifications to the card. `balanceOfCard` returns "amount owed" for `credito`/`deuda` (see `isDebtType`) and "amount held" otherwise; Resumen's net balance subtracts both debt kinds
- `categories`: each has a `freq` of `mensual` / `anual` / `esporadico` (drives how Resumen groups spending)
- `movements`: the transaction log — `type` (`gasto`/`ingreso`), `title` (required in the UI; older entries may only have `description`, so display falls back `title || description`), `description` (optional), `amount`, `commission`, `months` (>1 means MSI/installments), `categoryId`, `cardId`, `date`, and flags for system/special entries: `interest` (savings yield), `adjust` (balance edits), `transfer` + `transferId` (two-leg transfers between cards), `recurring` + `recurringId` (auto-generated monthly charges), `split` + `splitId` (split payments — see below)
- `recurring`: fixed charges/deposits — `title` (+ optional `description`), `amount`, optional `type` (`"gasto"` default = a charge/cargo, or `"ingreso"` = a recurring deposit/abono to any account, e.g. automatic savings), `cardId`, `categoryId`, frequency (`unit` `"mes"`/`"semana"` + `every` N; monthly uses `day` 1–31, weekly uses `weekday` 0–6; legacy items with only `day` default to monthly), optional `endDate`, `createdAt`, `lastApplied`. Helpers `recurringFreqLabel` and `monthlyEquivalent` in finance.js. `applyRecurring()` runs on load and when the list changes, generating overdue movements of the item's `type` (flagged `recurring`) up to today; generated movements count as normal spending/income in every stat, and deleting a recurring item keeps its past movements. Managed in the "Fijos" tab (shows separate monthly totals for cargos vs abonos)
- `theme`: `"dark"` or `"light"`
...
**Split payments** (pago dividido): a single purchase paid with several methods. In the Movimientos "+ Nuevo movimiento" form (gasto only), the "Pagar con varios medios" toggle (IconSplit) switches to split mode: one row per method `{cuenta, tarjeta, monto}`, the total is the sum. On save, `saveSplit` creates one normal `gasto` movement per method, all sharing a `splitId` and flagged `split: true` (a single-method split falls back to a plain movement). Each leg counts fully in every stat (so the legs sum to the purchase), and cash legs are cash-overdraft-guarded. In the movements list, legs with the same `splitId` are grouped into one row (`visibleRows`) showing the per-method breakdown; deleting it removes all legs (`del` handles `splitId` like `transferId`). Split rows are delete-only (no inline editor).
...
**Debt payments**: the Deudas section in Resumen has a "Pagar deuda" button that reuses the same pay flow as credit cards (`openPay`/`doPay`/`payEditor`) — it creates a transfer (`gasto` on the source card, `ingreso` on the `deuda` card) so the payment is a real movement/record and reduces the debt without counting as spending. Source options exclude `credito` and `deuda` cards. When a debt reaches a zero balance it shows "Liquidada" and an "Eliminar deuda" button (`deleteDebt`) that removes the debt account, its card, and its own movements (the `gasto` payment legs on other accounts are kept as records, so the balance is unaffected).
...
`src/App.jsx` uses `@capawesome/capacitor-app-update` (Google Play In-App Updates) to check on launch whether a newer version is on Play; if so it shows an "actualización disponible" banner above the tabs. Tapping it runs `performImmediateUpdate()` (Play's full-screen flow), falling back to `openAppStore()`. Only works for Play-installed builds (internal/closed/production), not sideloaded APKs; guarded by `Capacitor.isNativePlatform()` and try/catch so it's a no-op elsewhere.
